...
class FinanceLoader():
    ...
    def get_finan_report(self):
        ...
        page = requests.get(
            "https://finfo-api.vndirect.com.vn/v3/stocks/financialStatement?secCodes={}&reportTypes=QUARTER&modelTypes=1,89,101,411&fromDate={}&toDate={}".format(
    ...
    def get_business_report(self):
        start_time = time.time()
        page = requests.get(
            "https://finfo-api.vndirect.com.vn/v3/stocks/financialStatement?secCodes={}&reportTypes=QUARTER&modelTypes=2,90,102,412&fromDate={}&toDate={}".format(
                self.symbol, self.start, self.end))
        data = page.json()
        end_time = time.time()
        data_dates = {}
    ...
    def get_cashflow_report(self):
        start_time = time.time()
        page = requests.get(
            "https://finfo-api.vndirect.com.vn/v3/stocks/financialStatement?secCodes={}&reportTypes=QUARTER&modelTypes=3,91,103,413&fromDate={}&toDate={}".format(
                self.symbol, self.start, self.end))
        data = page.json()
        end_time = time.time()
        data_dates = {}
    ...
    def get_basic_index(self):
        ...
        for year in years:
            page = requests.get(
                "https://finfo-api.vndirect.com.vn/v4/ratios?q=code:{}~itemCode:53030,52005,51050,53021,52001,52002,54018,712010,712020,712030,712040~reportDate:{}".format(
                    self.symbol, year), headers=headers)
            data = page.json()
